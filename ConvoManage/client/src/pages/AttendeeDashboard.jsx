  ...
  <div className="attendee-container">
    ...
    {data.map(({ conf, sessions }) => (
      <div key={conf._id} className="conference-card">
        <h4>{conf.title}</h4>
        <p><em>{new Date(conf.date).toLocaleDateString()}</em></p>
        ...
        {sessions.length > 0 ? (
            ...
            <ul>
              {sessions.map((s) => (
                <li key={s._id} className="session-item">
                  <strong>{s.title}</strong> by {s.speaker?.name || "Unknown"}<br />
                  ...
                  {registered.includes(s._id) ? (
                    ...
                    <button onClick={() => handleRegister(s._id)}>Register</button>
                  )}
                  <button onClick={() => handleJoinSession(s._id)}>Join</button>
          ...
          <p><em>No sessions scheduled yet.</em></p>
        )}
      </div>
    ))}
  </div>
